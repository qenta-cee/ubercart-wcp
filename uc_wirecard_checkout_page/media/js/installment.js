...
$(document).ready(function() {
    ...
    $("#wcp_installment_payolution_terms").wrap('<label>').prepend(' <input type="checkbox" name="wcp_installment_payolution_terms">');

    $("#edit-continue").click(function(a){
        if($("#edit-panes-payment-payment-method-uc-wcp-installment").is(":checked")){
            var dob = new Date($("#wcp_installment_datepicker").datepicker('getDate')),
                underage = Math.abs(new Date(Date.now() - dob.getTime()).getUTCFullYear() - 1970) < 18,
                ptos = $("input[name=wcp_installment_payolution_terms]");
            if(underage){
                $("#wcp_installment_too_young").show();
                $('html, body').animate({
                    scrollTop: $("#wcp_installment_too_young").offset().top-150
                }, 'fast');
                a.preventDefault();
                return false;
            }
            else
                $("#wcp_installment_too_young").hide();

            if(ptos.length > 0 && !ptos.is(":checked")) {
                $("#wcp_installment_payolution_terms_not_checked").show();
                $('html, body').animate({
                    scrollTop: $("#wcp_installment_payolution_terms_not_checked").offset().top-150
                }, 'fast');
                a.preventDefault();
                return false;
            }
            else
                $("#wcp_installment_payolution_terms_not_checked").hide();
        }
